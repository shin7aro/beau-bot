// spell-map.js
// Server-side copy of the SPELL_MAP living in public/js/spell-map.js
// (frontend, browser-only — same manually-kept-in-sync-duplicate
// convention as item-map.js/public/js/item-map.js; see that file's
// header for why).
//
// Maps every weapon/head/chest/feet name in item-map.js's ITEM_MAP to
// its Albion spell "rows" — each group is a mutually-exclusive choice
// (Albion only lets one spell per row be active at a time), with the
// candidates for that row, and to its real Albion weapon/armor
// subcategory (e.g. "sword", "plate_helmet" — from items.xml's own
// @shopsubcategory1, not guessed from the name), used by the Spell
// Picker page to group gear the same way Albion itself does.
//
// THIRD (E) ABILITY DATA SOURCE (weapons): items.xml's own
// craftingspelllist does NOT capture each weapon's real third ability
