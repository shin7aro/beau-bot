...
// candidates for that row and each one's render.albiononline.com icon
// identifier (which is NOT always the spell's own unique name — see
// the "icon" field on each candidate; render_url is built as
// `https://render.albiononline.com/v1/spell/${icon}.png`).
//
// Offhands are intentionally absent: verified against the game's own
// binary dump that every offhand has 0 spell slots, so there is
// nothing to choose. Capes/food/potions were never spell-bearing.
//
...
// category per spell) on 2026-09-14. If item-map.js's ITEM_MAP gains a
// new weapon/head/chest/feet entry, or Albion changes a weapon's
