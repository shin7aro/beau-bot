/* ─────────────────────────────────────────
   ITEM MAP — shared frontend item name -> Albion
   render ID lookup, plus the imgUrl() helper built
   on top of it. Used by both builds.js (build cards,
   item picker) and events.js (roster pill icons,
   linked-build gear grid) — load this script before
   either of those.

   NOTE: there is also a server-side copy of this map
   in item-map.js at the project root (index.js can't
   reach browser code directly). Keep both in sync when
   adding items — see that file's header for details.
───────────────────────────────────────── */
window.ITEM_MAP = {
  // Weapons
...
// and events.js each declare for their own local use.
window.imgUrl = function imgUrl(name) {
  const id = window.ITEM_MAP[name];
